...
from card_detector import find_cards_on_table
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for i, (player, name, stack) in enumerate(zip(players, names, stacks)):
            ...
            if player.leaving:
                player.leave_table()
            # Calculate pot odds if the player is facing a bet
            if player.action and player.action.startswith(("Call", "Bet", "Raise")):
                pot_odds = player.calculate_pot_odds(pot, player.action)
                print(f"{player.name}'s pot odds: {pot_odds}")
        ...
        data = {
            ...
            'Pot Odds': [
                player.calculate_pot_odds(pot, player.action)
                if player.action and player.action.startswith(("Call", "Bet", "Raise"))
                else None
                for player in players
            ],
